...
#
# Usage:
#   ./scripts/dev-start.sh              # All services (AI Evals via Docker)
#   ./scripts/dev-start.sh --no-evals   # Skip AI Evals
#   ./scripts/dev-start.sh --neo4j      # Also start Neo4j for FFX
#   ./scripts/dev-start.sh --evals-local # Run AI Evals Flask directly (no Docker)
# ──────────────────────────────────────────────────────────
...
START_EVALS=true
EVALS_MODE="docker"   # "docker" or "local"
START_NEO4J=false
PIDS=()
...
for arg in "$@"; do
  case "$arg" in
    ...
    --evals-local) EVALS_MODE="local" ;;
    -h|--help)
      echo "Usage: ./scripts/dev-start.sh [--no-evals] [--neo4j] [--evals-local]"
      echo ""
      echo "  --no-evals      Skip AI Evals app entirely"
      echo "  --neo4j         Start Neo4j for FFX (default: skip, uses mock data)"
      echo "  --evals-local   Run AI Evals Flask directly instead of via Docker"
      exit 0
      ;;
  esac
done
...
# ── AI Evals local mode (no Docker) ─────────────────────

if [ "$START_EVALS" = true ] && [ "$EVALS_MODE" = "local" ]; then
  echo "Starting AI Evals Flask app (local mode)..."
  ...
  if [ ! -f "$EVALS_DIR/.env" ]; then
    if [ -f "$EVALS_DIR/.env.example" ]; then
      cp "$EVALS_DIR/.env.example" "$EVALS_DIR/.env"
      echo "  Warning: Created .env from example — add your ANTHROPIC_API_KEY"
    fi
  fi
