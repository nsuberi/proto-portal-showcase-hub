...
Some prototypes in this monorepo may maintain a persistent affordances file (e.g., `affordances.md` within the prototype directory) as a living bill of materials with tables for Places, UI Affordances (U-prefixed), Code Affordances (N-prefixed), Data Affordances (D-prefixed), Wiring, and Component Templates.

### Loading the Reference

At the start of any breadboarding session:
1. Check if the relevant prototype has an `affordances.md` file
2. Read it to understand what already exists
3. Cross-reference existing IDs (U, N, D prefixes) to avoid conflicts
4. Note the highest ID in each sequence so new affordances continue the numbering
