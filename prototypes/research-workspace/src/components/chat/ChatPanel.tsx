...
function AuthPrompt({ authUrl, isAuthenticating, onStartAuth }: {
  authUrl: string | null;
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-6">
      ...
      {authUrl ? (
        <>
          <p className="font-body text-sm text-on-surface-variant/60 max-w-sm mb-4">
            Click the link below to sign in with your Anthropic account. Once authenticated, you can start chatting.
          </p>
          <a
            href={authUrl}
            target="_blank"
        ...
        <>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 font-label text-sm px-5 py-2.5 rounded-lg bg-primary text-on-primary hover:bg-primary/90 transition-colors disabled:opacity-50"
          >
            {isAuthenticating ? (
            ...
            {isAuthenticating ? "Connecting..." : "Connect Claude"}
          </button>
        </>
      )}
    </div>
...
export default function ChatPanel() {
  const { messages, isStreaming, isConnected, authUrl, isAuthenticating, sendMessage, startAuth } = useChat();
    ...
    <div className="h-full flex flex-col bg-surface-container-lowest">
      ...
      <div className="flex-1 overflow-y-auto">
        {authUrl || isAuthenticating ? (
          <AuthPrompt authUrl={authUrl} isAuthenticating={isAuthenticating} onStartAuth={startAuth} />
        ) : messages.length === 0 ? (
